...
- Health: a second `aws_sns_topic` with the same recipients and no key, an `aws_sqs_queue`
  holding alerts that could not be delivered, and five `aws_cloudwatch_metric_alarm` instances
  reporting to the health topic: one per rule and two for the channel.
...
1. Someone calls a security group, IAM, or CloudTrail write API. CloudTrail records the call.
   IAM is a global service, so CloudTrail records its calls in the partition's global-service
   region: `us-east-1` in the commercial partition, `us-gov-west-1` in GovCloud. Security group
   calls are recorded in the region they target. A trail's own stop and change calls are
   accepted only in that trail's home region. The provider must therefore target the
   global-service region, the workloads must live there too, and the trail must be homed there;
   the IAM rule refuses any other region at plan.
...
`tests/alerts.tftest.hcl`, and adding its fixtures under `tools/fixtures/events/`. Adding a
region is a code change across providers, locals, resources and tests, as it is in every
...
already failed. The alarms watch the three ways delivery breaks, one per rule for the first:
...
an alarm about a broken alert topic cannot be delivered through that topic. The health topic is
also unencrypted, for the same reason: a broken key policy is one of the failures it reports, so
the report must not travel through the key, and what it carries is an alarm's name and state.
Its policy admits publishes from this deployment's own alarms, named by ARN, and nothing else.
Each of those metrics is published only when it is non-zero, so the alarms treat missing data as
healthy.
